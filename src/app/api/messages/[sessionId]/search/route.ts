...
export async function GET(
    ...
    try {
        ...
        if (type) {
            ...
            if (!Object.values(MessageType).includes(normalizedType as MessageType)) {
                return NextResponse.json({ status: false, message: "Invalid message type", error: "Invalid message type" }, { status: 400 });
            }
            where.type = normalizedType as MessageType;
        }
