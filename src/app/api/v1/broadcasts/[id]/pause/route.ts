...
import { pauseBroadcast } from "@/lib/broadcast-service";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return apiV1Success(requestId, await pauseBroadcast({ id: user.id, role: user.role, ownerId: user.ownerId, apiKeyId: user.apiKeyId }, id));
  } catch (error) {
