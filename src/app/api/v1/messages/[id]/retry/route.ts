...
import { retryMessageJob } from "@/lib/message-job-service";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return apiV1Success(requestId, await retryMessageJob({ id: user.id, role: user.role, ownerId: user.ownerId, apiKeyId: user.apiKeyId }, id), 202);
  } catch (error) {
