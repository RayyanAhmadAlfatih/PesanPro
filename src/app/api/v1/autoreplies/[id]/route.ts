...
export async function PUT(request: NextRequest, { params }: Context) {
  ...
  try {
    ...
    const result = await updateAutoReplyRule(user, (await params).id, parseAutoReplyBody(await request.json()));
    return apiV1Success(requestId, result.rule, 200, { warnings: result.warnings });
...
export async function DELETE(request: NextRequest, { params }: Context) {
  ...
  try {
    ...
    return apiV1Success(requestId, await deleteAutoReplyRule(user, sessionId, (await params).id));
  } catch (error) { return apiV1Exception(requestId, error); }
}
