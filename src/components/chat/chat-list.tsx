...
function LabelAssignPopover({ sessionId, jid, children }: { sessionId: string; jid: string; children: React.ReactNode }) {
    ...
    const fetchLabels = useCallback(async () => {
        ...
        try {
            ...
            if (assignedRes.ok) {
                const assignments = Array.isArray(assignedData.data) ? assignedData.data : [];
                setAssigned(new Set(assignments.flatMap((assignment: unknown) => {
                    return typeof assignment === "object" && assignment !== null && "labelId" in assignment && typeof assignment.labelId === "string"
                        ? [assignment.labelId]
                        : [];
                })));
            }
...
export function ChatList({ sessionId, onSelectChat, selectedJid }: ChatListProps) {
    ...
    const fetchChats = useCallback(async (cursor?: string, append = false) => {
        ...
        try {
            ...
            const rawChats = await getChatsStatus(sessionId, PAGE_SIZE, cursor || undefined, searchQuery || undefined);
    ...
    useEffect(() => {
        ...
        const handler = async (newMessages: ChatMessageUpdate[]) => {
