...
export function ChatWindow({ sessionId, jid, name, onBack }: ChatWindowProps) {
    ...
    const fetchMessages = useCallback(async (before?: string) => {
        try {
            ...
            const data = await getChatMessages(sessionId, jid, PAGE_LIMIT, before);
    ...
    const handleSend = async () => {
        ...
        } catch (e: unknown) { toast.error(getErrorMessage(e) || "Failed to send"); }
    };
    ...
    const processFileUpload = async (file: File, explicitType?: string) => {
        ...
        try {
            ...
            toast.success("Media queued for delivery");
            setTimeout(() => fetchMessages(), 800);
        } catch (error: unknown) { toast.error(getErrorMessage(error) || "Failed to send media"); }
    };
