...
export default function WebhookLogDialog({ webhookId, webhookName, targetSessionId, open, onClose }: Props) {
    ...
    const replayDelivery = async (deliveryId: string) => {
        if (!window.confirm("Replay this event to the same endpoint? A new audited delivery will be queued.")) return;
        try {
            const response = await fetch(`/api/webhooks/${targetSessionId}/${webhookId}/deliveries/${deliveryId}/replay`, { method: "POST" });
            if (!response.ok) throw new Error("Replay request failed");
            toast.success("Webhook replay queued");
            await fetchLogs();
        } catch (error) {
            toast.error(error instanceof Error ? error.message : "Webhook replay failed");
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
            <DialogContent className="max-w-5xl sm:max-w-5xl w-[96vw] h-[82vh] max-h-[85vh] flex flex-col p-0 overflow-hidden gap-0 bg-background shadow-none border border-slate-200/80">
                        ...
                        <div className={cn(
                            ...
                            <div className="p-3 px-4 border-b flex items-center justify-between text-xs text-muted-foreground shrink-0 bg-slate-100/30">
                                <span className="flex items-center gap-2">
                                    ...
                                    <span className="bg-[var(--pp-mint)] text-[var(--pp-ink)] px-2 py-0.5 rounded-full flex items-center gap-1 font-semibold text-[10px] border border-[var(--pp-line)]">
                                        <span className="h-1 w-1 rounded-full bg-[var(--pp-mint)]  inline-block" />
                                        Live
                                    </span>
                                </span>
                                            ...
                                            <div className="flex items-center gap-2.5 mt-0.5">
                                                <div className="flex items-center gap-1.5">
                                                    <span className={cn(
                                                        ...
                                                        isSuccess ? "bg-[var(--pp-mint)]" : "bg-rose-500"
                                                    )} />
                                                    <span className={cn(
                                                        "text-[10px] font-semibold uppercase tracking-wider",
                                                        isSuccess ? "text-[var(--pp-ink)]" : "text-rose-600"
                                                    )}>
                                                ...
                                                {log.responseStatusCode != null && (
                                                    <span className={cn(
                                                        ...
                                                        isStatusError ? "text-rose-600 border-rose-100" : "text-[var(--pp-ink)] border-[var(--pp-line)]"
                                                    )}>
                                    ...
                                    <div className="p-4 px-5 border-b flex flex-col gap-3 bg-slate-50/10 shrink-0">
                                        ...
                                        <div className="flex items-center justify-between gap-2.5 flex-wrap">
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 text-xs">
                                                ...
                                                <Badge
                                                    variant={["SUCCESS", "SUCCEEDED"].includes(selectedLog.status) ? "default" : "destructive"}
                                                    className={cn(
                                                        "px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider",
                                                        ["SUCCESS", "SUCCEEDED"].includes(selectedLog.status) ? "bg-[var(--pp-ink)] hover:bg-[var(--pp-ink)] text-white" : ""
                                                    )}
                                                >
                                                    {selectedLog.status}
                                                </Badge>
                                                ...
                                                {selectedLog.responseStatusCode != null && (
                                                    <span className={cn(
                                                        ...
                                                        selectedLog.responseStatusCode >= 400 ? "text-rose-600 border-rose-100/50" : "text-[var(--pp-ink)] border-[var(--pp-line)]"
                                                    )}>
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => replayDelivery(selectedLog.id)}
                                                >
