...
class AntiSpamManager {
     ...
     */
    async enqueue(sessionId: string, jid: string, content: AnyMessageContent): Promise<void> {
    ...
    private detectMessageType(content: AnyMessageContent): string {
        ...
        if ("delete" in content) return "delete";
        if ("poll" in content) return "poll";
        if ("location" in content) return "location";
        if ("contacts" in content) return "contact";
        return "other";
    }
